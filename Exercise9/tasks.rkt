...
;make function to check if it is correct arithmetic-tree
...
(define (calculate tree)
    (let ((r (root tree)))
      (if (procedure? r)
          (r (calculate (left-tree tree)) (calculate (right-tree tree)))
          r)))
...
(define (contains? bst elem)
  (cond ((empty-tree? bst) #f)
        ...
        ((> elem (root bst))  (contains? (right-tree bst) elem))))

(define (insert bst elem)
  (cond ((empty-tree? bst) (make-leaf elem))
        ((< elem (root bst)) (make-tree (root bst) (insert (left-tree bst) elem) (right-tree bst)))
        (else (make-tree (root bst) (left-tree bst) (insert (right-tree bst) elem)))))
